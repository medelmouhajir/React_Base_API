    ...
    return (
        <div className={`relative ${className}`} ref={dropdownRef}>
            <button
                ...
                className={`theme-toggle ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-700'}`}
            ...
            {/* Theme dropdown */}
            {dropdownOpen && (
                <div className={`dropdown-content mt-2 w-40 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                    <div className="py-1">
                        {themeOptions.map(option => (
                            <button
                                key={option.id}
                                type="button"
                                className={`dropdown-item flex items-center w-full ${theme === option.id ? 'font-semibold ' + (isDarkMode ? 'text-primary-400' : 'text-primary-600') : ''}`}
                                onClick={() => {
