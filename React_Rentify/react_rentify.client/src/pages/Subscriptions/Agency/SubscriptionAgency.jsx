...
const SubscriptionAgency = () => {
    ...
    useEffect(() => {
        const loadSubscriptionData = async () => {
            ...
            try {
                ...
                setError(t('subscription.agency.loadError'));
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSubscribe = async (planId, startTrial = false) => {
        ...
        try {
            ...
            setError(t('subscription.agency.subscribeError'));
        } finally {
    ...
    const handleUpgrade = async () => {
        ...
        try {
            ...
            setError(t('subscription.agency.upgradeError'));
        } finally {
            setIsOperating(false);
        }
    };
    ...
    const handleCancel = async () => {
        if (!window.confirm(t('subscription.agency.confirmCancel'))) return;
        ...
        try {
            ...
            setError(t('subscription.agency.cancelError'));
        } finally {
            setIsOperating(false);
        }
    };
    ...
    const handleResume = async () => {
        ...
        try {
            ...
            setError(t('subscription.agency.resumeError'));
        } finally {
            setIsOperating(false);
        }
    };
        ...
        return (
            <div className="subscription-agency-container">
                <div className="error-state">
                    ...
                    <h3>{t('subscription.agency.errorTitle')}</h3>
                    <p>{error}</p>
        ...
        <div className="subscription-agency-container">
            ...
            <div className="page-header">
                <div className="header-content">
                    <h1 className="page-title">{t('subscription.agency.title')}</h1>
                    <p className="page-subtitle">{t('subscription.agency.subtitle')}</p>
                </div>
            </div>
            ...
            {subscription ? (
                <div className="subscription-overview-card">
                    <div className="card-header">
                        <h2 className="card-title">{t('subscription.agency.currentPlan')}</h2>
                    ...
                    <div className="card-content">
                        <div className="subscription-details">
                            <div className="detail-group">
                                <label className="detail-label">{t('subscription.agency.planName')}</label>
                                <span className="detail-value">{subscription.subscriptionPlan?.name}</span>
                            </div>
                            <div className="detail-group">
                                <label className="detail-label">{t('subscription.agency.price')}</label>
                                <span className="detail-value">
                                    {formatCurrency(subscription.currentPrice)}
                                    / {t(`subscription.billingCycle.${subscription.subscriptionPlan?.billingCycle === 1 ? 'monthly' : 'yearly'}`)}
                                </span>
                            </div>
                            <div className="detail-group">
                                <label className="detail-label">{t('subscription.agency.nextBilling')}</label>
                            ...
                            {subscription.isTrialPeriod && (
                                <div className="detail-group trial-info">
                                    <label className="detail-label">{t('subscription.agency.trialEnds')}</label>
                ...
                <div className="no-subscription-state">
                    ...
                    <h3>{t('subscription.agency.noSubscription')}</h3>
                    <p>{t('subscription.agency.noSubscriptionDescription')}</p>
                </div>
            )}
            ...
            {availablePlans.length > 0 && (
                <div className="plans-section">
                    <h2 className="section-title">
                        {subscription ? t('subscription.agency.availablePlans') : t('subscription.agency.choosePlan')}
                    </h2>
                                ...
                                <div className="plan-header">
                                    ...
                                    <div className="plan-price">
                                        ...
                                        <span className="price-period">
                                            / {t(`subscription.billingCycle.${plan.billingCycle === 1 ? 'monthly' : 'yearly'}`)}
                                        </span>
                                    </div>
                                </div>
                                <div className="plan-content">
                                    ...
                                    <div className="plan-features">
                                        ...
                                        <ul className="features-list">
                                            <li>{t('subscription.agency.maxCars', { count: plan.maxCars })}</li>
                                            <li>{t('subscription.agency.maxUsers', { count: plan.maxUsers })}</li>
                                            <li>{t('subscription.agency.maxCustomers', { count: plan.maxCustomers })}</li>
                                            <li>{t('subscription.agency.maxReservations', { count: plan.maxReservations })}</li>
                                            {plan.hasMaintenanceModule && <li>{t('subscription.agency.maintenanceModule')}</li>}
                                            {plan.hasExpenseTracking && <li>{t('subscription.agency.expenseTracking')}</li>}
                                            {plan.hasAdvancedReporting && <li>{t('subscription.agency.advancedReporting')}</li>}
                                            {plan.hasAPIAccess && <li>{t('subscription.agency.apiAccess')}</li>}
                                            {plan.hasGPSTracking && <li>{t('subscription.agency.gpsTracking')}</li>}
                                        </ul>
                                    </div>
                                </div>
                                    ...
                                    {!subscription ? (
                                        <div className="subscribe-buttons">
                                            <button
                                                ...
                                                disabled={isOperating}
                                            >
                                                {t('subscription.agency.startTrial')}
                                            </button>
                                            <button
                                                className="btn-primary"
                                                ...
                                                disabled={isOperating}
                                            >
                                                {t('subscription.agency.subscribe')}
                                            </button>
                                        </div>
                                        ...
                                        <button
                                            ...
                                            disabled={isOperating}
                                        >
                                            {t('subscription.agency.switchToPlan')}
                                        </button>
                                    )}
                                </div>
                            </div>
            ...
            {subscription && currentUsage && (
                <div className="usage-overview-card">
                    <h2 className="card-title">{t('subscription.agency.currentUsage')}</h2>
                    <div className="usage-grid">
                        <div className="usage-item">
                            <label className="usage-label">{t('subscription.agency.carsUsed')}</label>
                        ...
                        <div className="usage-item">
                            <label className="usage-label">{t('subscription.agency.usersUsed')}</label>
                            <div className="usage-bar">
                                <div className="usage-progress" style={{
                        ...
                        <div className="usage-item">
                            <label className="usage-label">{t('subscription.agency.customersUsed')}</label>
                            <div className="usage-bar">
                                <div className="usage-progress" style={{
            ...
            {invoices.length > 0 && (
                <div className="invoices-section">
                    <h2 className="section-title">{t('subscription.agency.recentInvoices')}</h2>
                    <div className="invoices-placeholder">
                        ...
                        <p>{t('subscription.agency.invoicesComingSoon')}</p>
                    </div>
                </div>
            )}
                ...
                <div className="modal-overlay" onClick={() => setShowPlanUpgrade(false)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        <div className="modal-header">
                            <h3 className="modal-title">{t('subscription.agency.confirmUpgrade')}</h3>
                        ...
                        <div className="modal-body">
                            <p>{t('subscription.agency.upgradeConfirmation')}</p>
                        </div>
                        <div className="modal-actions">
                            ...
                            <button
                                ...
                                disabled={isOperating}
                            >
                                {isOperating ? t('common.processing') : t('subscription.agency.confirmUpgrade')}
                            </button>
                        </div>
                    </div>
                </div>
