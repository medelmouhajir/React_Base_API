...
const Viewer = () => {
    ...
    const ZOOM_STEP = 100;
    const PAGES_AHEAD = 3; // How many pages to load ahead
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Initialize visible pages for infinite scroll
                if (sortedPages.length > 0) {
                    const initialCount = Math.min(5, sortedPages.length);
                    setVisiblePages(sortedPages.slice(0, initialCount));
                    setLastLoadedIndex(initialCount - 1);
                }
            } catch (error) {
    ...
    // Setup intersection observer for infinite scroll lazy loading
    const lastPageElementRef = useCallback(node => {
        ...
        observerRef.current = new IntersectionObserver(entries => {
            if (entries[0].isIntersecting && lastLoadedIndex < pages.length - 1) {
                // When last visible page is intersecting, load more pages
                const nextIndex = lastLoadedIndex + 1;
                const loadUntilIndex = Math.min(nextIndex + PAGES_AHEAD, pages.length - 1);

                const newVisiblePages = [
                    ...visiblePages,
                    ...pages.slice(nextIndex, loadUntilIndex + 1)
                ];

                setVisiblePages(newVisiblePages);
                setLastLoadedIndex(loadUntilIndex);
            }
        }, { threshold: 0.1 });

        if (node && mode === 'infinite') observerRef.current.observe(node);
    }, [loading, lastLoadedIndex, pages, visiblePages, mode]);

    // Reset visible pages when mode changes
    useEffect(() => {
        if (mode === 'infinite' && pages.length > 0) {
            const initialCount = Math.min(5, pages.length);
            setVisiblePages(pages.slice(0, initialCount));
            setLastLoadedIndex(initialCount - 1);
        }
    }, [mode, pages]);
    ...
    const renderInfiniteMode = () => (
        <div className="infinite-mode">
            {visiblePages.map((page, index) => {
                const isLastElement = index === visiblePages.length - 1;

                return (
                    <img
                        key={page.id}
                        ref={isLastElement ? lastPageElementRef : null}
                        src={`${import.meta.env.VITE_API_URL}${page.imageUrl}`}
                        alt={`Page ${pages.findIndex(p => p.id === page.id) + 1}`}
                        className="manga-page"
                        loading="lazy"
                    />
                );
            ...
            {lastLoadedIndex < pages.length - 1 && (
