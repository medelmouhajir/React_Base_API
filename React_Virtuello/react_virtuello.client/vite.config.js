...
let httpsConfig = {};
let target = 'https://localhost:7221';
...
if (!isDocker) {
    ...
    const certificateName = "react_virtuello.client";
    ...
    target = process.env.ASPNETCORE_HTTPS_PORT ? `https://localhost:${process.env.ASPNETCORE_HTTPS_PORT}` :
        process.env.ASPNETCORE_URLS ? process.env.ASPNETCORE_URLS.split(';')[0] : 'https://localhost:7221';
}
...
export default defineConfig({
    ...
    server: {
        host: '0.0.0.0',
        allowedHosts: true,
        port: parseInt(process.env.DEV_SERVER_PORT || '53256'),
